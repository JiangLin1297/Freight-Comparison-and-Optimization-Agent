...
class FreightRate(Base):
    ...
    min_weight = Column(Float, nullable=False, comment="最小重量(kg)")
    max_weight = Column(Float, nullable=False, comment="最大重量(kg)")
    service_level = Column(String(10), nullable=False, comment="服务级别(DTD/DTP)")
    min_cost = Column(Float, nullable=False, comment="最低费用($)")
    rate = Column(Float, nullable=False, comment="单价($/kg)")
